...
namespace RingCentralDataIntegration
{
    internal class DataHandler
    {
        internal static SqlConnection Connection()
        {
            ...
            string connetionString = ConfigurationManager.ConnectionStrings["partialConnectString"] + $";User ID={dbUserName};Password={dbPassword}"; //$"Data Source=52.6.249.188;Initial Catalog=TechBrands;User ID={dbUserName};Password={dbPassword}";
        ...
        internal static void CallLegToDatebase(DataSet ds)
        {
            ...
            //table.Columns.Add("DeviceFromID");
            //table.Columns.Add("DeviceToID");
            table.Columns.Add("DateAccessed");
            ...
            //var deviceTblIndex = ds.Tables.IndexOf("device");
            ...
            for (var i = 0; i <= ds.Tables[tblIndex].Rows.Count - 1; i++)
            {
                ...
                //var fromId = Convert.ToInt32(ds.Tables[fromTblIndex].Rows[i]["from_Id"]);
                ...
                if (callTo.Length > 0)
                {
                    //row[11] = callTo[0]["extensionNumber"];
                ...
                {
                    //row[15] = callFrom[0]["extensionNumber"];
                    row[14] = callFrom[0]["name"].ToString().Replace("'", "''");
                    row[15] = callFrom[0]["phoneNumber"];
                    row[16] = callFrom[0]["location"].ToString().Replace("'", "''");
                }

                //var deviceFrom = ds.Tables[deviceTblIndex].Select("from_Id = " + fromId);
                //if (deviceFrom.Length > 0)
                //{
                //    row[20] = deviceFrom[0]["id"];
                //}
        ...
        internal static void ExtensionToDatabase(XmlDocument response) // 2 seconds
        {
            var ds = new DataSet();
            var xr = new XmlNodeReader(response);
            var dateAccessed = DateTime.Now;
            ...
            //var tblIndex = ds.Tables.IndexOf("records");
            //var contTblIndex = ds.Tables.IndexOf("contact");
            //var permTblIndex = ds.Tables.IndexOf("permissions");
            //var pageTblIndex = ds.Tables.IndexOf("paging");
            //var adminTblIndex = ds.Tables.IndexOf("admin");
            //var intCallingTblIndex = ds.Tables.IndexOf("internationalCalling");

            for (var i = 0; i <= ds.Tables[1].Rows.Count - 1; i++)
            {
                var row = table.NewRow();
                var columns = ds.Tables["records"].Columns;
                //var rows = ds.Tables["records"].Rows[i];
                //var contactsColumns = ds.Tables["contact"].Columns;
                var permissionsColumns = ds.Tables["permissions"].Columns;
                //var adminColumns = ds.Tables["admin"].Columns;
                //var intCallingColumns = ds.Tables["internationalCalling"].Columns;
                //var pagingColumns = ds.Tables["paging"].Columns;
        ...
        internal static void ExtensionPhoneListToDatabase(XmlDocument response) // 0 seconds
        {
            var ds = new DataSet();
            var xr = new XmlNodeReader(response);
            var table = new DataTable();
        ...
        internal static void ReadXmlToDatabase(XmlDocument response, string tableName, DateTime date)
        {
            ...
            var tbl = (ds.Tables["records"] != null)
                ? "records"
                : (ds.Tables["record"] != null)
                    ? "record"
                    : (ds.Tables["Table1"] != null) ? "Table1" : "";

            //ds.Tables[tbl].Columns.Add("DivisionID");
            ds.Tables[tbl].Columns.Add("AccessedDate");

            for (var i = 0; i < ds.Tables[tbl].Rows.Count; i++)
            {
                //ds.Tables[tbl].Rows[i]["DivisionID"] = divisionId;
                ds.Tables[tbl].Rows[i]["AccessedDate"] = date;
            }

            foreach (var column in ds.Tables[tbl].Columns)
            {
                if (!columns.Contains(column.ToString()))
                {
                    AddColumnToTable(tableName, column.ToString());
                    SmtpHandler.SendMessage($"RQ API import added column {tableName}.", $"Column {column} added to {tableName} because it did not exist in the staging table and came through for.");
                }
            }

            using (var bulkCopy = new SqlBulkCopy(connection))
            {
                Console.WriteLine($"Attempting to save data to {tableName} SQL table for.");

                connection.Open();
                foreach (DataColumn column in ds.Tables[tbl].Columns)
                {
                ...
                bulkCopy.BatchSize = 10000;
                bulkCopy.WriteToServer(ds.Tables[tbl]);
                connection.Close();

                Console.WriteLine($"Successfully saved data to {tableName} SQL table for :D");
            }
            //}
            //catch (Exception e)
            //{
